...
namespace Arasan.Controllers
{
    public class PrivilegesController : Controller
    {
        ...
        public ActionResult MyListPrivgrid(string strStatus)
        {
            ...
            dtUsers=datatrans.GetData("select E.EMPNAME,DS.DESIGNATION,D.DEPARTMENT,P.PRIVILEGESID,P.IS_ACTIVE from USER_PRIVILEGES P,EMPMAST E,PDESG DS,PDEPT D WHERE P.EMPID=E.EMPMASTID AND P.PDESG_ID=DS.PDESGID AND P.PDEPT_ID=D.PDEPTID AND P.IS_ACTIVE='Y'");
            for (int i = 0; i < dtUsers.Rows.Count; i++)
            {
                ...
                if (dtUsers.Rows[i]["IS_ACTIVE"].ToString() == "Y")
                {
                    ...
                    DeleteRow = "<a href=Deletepriv?tag=Del&id=" + dtUsers.Rows[i]["PRIVILEGESID"].ToString() + "><img src='../Images/Inactive.png' alt='Deactivate' /></a>";
                }
                else
                {
                    EditRow = "";
                    DeleteRow = "<a href=Deletepriv?tag=Active&id=" + dtUsers.Rows[i]["PRIVILEGESID"].ToString() + "><img src='../Images/active.png' alt='Activate' /></a>";
                }
                ...
                {
                    id = Convert.ToInt32(dtUsers.Rows[i]["PRIVILEGESID"].ToString()),
        ...
        public ActionResult ViewPriv(string id)
        {
            ...
            dtm = datatrans.GetData("select E.EMPNAME,DS.DESIGNATION,D.DEPARTMENT,P.PRIVILEGESID,P.IS_ACTIVE from USER_PRIVILEGES P,EMPMAST E,PDESG DS,PDEPT D WHERE P.EMPID=E.EMPMASTID AND P.PDESG_ID=DS.PDESGID AND P.PDEPT_ID=D.PDEPTID AND P.PRIVILEGESID='"+ id +"'");
            if(dtm.Rows.Count > 0)
            {
        ...
        public List<SelectListItem> BindDept()
        {
            try
            {
                ...
                dtDesg = datatrans.GetData("select DEPARTMENT,PDEPTID from PDEPT WHERE IS_ACTIVE='Y'");
                List<SelectListItem> lstdesg = new List<SelectListItem>();
        ...
        public JsonResult GetempJSON(string desgid)
        {
            return Json(Bindemp(desgid));
        }
        public List<SelectListItem> Binddesignation(string deptid)
        {
            ...
            {
                DataTable dtDesg = new DataTable();
                dtDesg = datatrans.GetData("select DESIGNATION,PDESGID from PDESG WHERE IS_ACTIVE='Y' AND PDEPTID='"+ deptid + "'");
                List<SelectListItem> lstdesg = new List<SelectListItem>();
                for (int i = 0; i < dtDesg.Rows.Count; i++)
                {
                    lstdesg.Add(new SelectListItem() { Text = dtDesg.Rows[i]["DESIGNATION"].ToString(), Value = dtDesg.Rows[i]["PDESGID"].ToString() });
                }
                return lstdesg;
            }
            catch (Exception ex)
        ...
        public List<SelectListItem> Bindemp(string desgid)
        {
            try
            {
                DataTable dtDesg = new DataTable();
                dtDesg = datatrans.GetData("select EMPMASTID,EMPNAME from EMPMAST where PDESGID='" + desgid + "'");
                List<SelectListItem> lstdesg = new List<SelectListItem>();
                for (int i = 0; i < dtDesg.Rows.Count; i++)
                {
