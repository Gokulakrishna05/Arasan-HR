    ...
    public class StateController : Controller
    {
        ...
        public List<SelectListItem> BindCountry()
        {
            try
            {
                ...
                {
                    lstdesg.Add(new SelectListItem() { Text = dtDesg.Rows[i]["COUNTRY"].ToString(), Value = dtDesg.Rows[i]["CONMASTID"].ToString() });
                }
                return lstdesg;
            }
            catch (Exception ex)
        ...
        public ActionResult MyListItemgrid(string strStatus)
        {
            ...
            for (int i = 0; i < dtUsers.Rows.Count; i++)
            {
                ...
                if (dtUsers.Rows[i]["IS_ACTIVE"].ToString() == "Y")
                {
                    ...
                    DeleteRow = "DeleteMR?tag=Del&id=" + dtUsers.Rows[i]["STATEMASTID"].ToString() + "";
                }
                else
                {

                    EditRow = "";
                    DeleteRow = "Remove?tag=Del&id=" + dtUsers.Rows[i]["STATEMASTID"].ToString() + "";
                }
