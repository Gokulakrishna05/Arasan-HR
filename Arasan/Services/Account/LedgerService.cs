...
namespace Arasan.Services
{
    public class LedgerService : ILedger
    {
        ...
        public DataTable GetAllListDayBookItem(string strfrom, string strTo)
        {
            ...
            {
                SvSql = "SELECT T1VCHNO,to_char(TRANS1.T1VCHDT,'dd-MON-yyyy')T1VCHDT,TRANS1.TRANS1ID,T1TYPE,T1NARR,DBCR,MID,DBAMOUNT,CRAMOUNT FROM TRANS1 INNER JOIN  TRANS2 ON TRANS1.TRANS1ID=TRANS2.TRANS1ID WHERE TRANS1.T1VCHDT BETWEEN '" + strfrom + "'  AND '" + strTo + "'";
            }
            else
            {
                SvSql = "SELECT T1VCHNO,to_char(TRANS1.T1VCHDT,'dd-MON-yyyy')T1VCHDT,TRANS1.TRANS1ID,T1TYPE,T1NARR,DBCR,MID,DBAMOUNT,CRAMOUNT FROM TRANS1 INNER JOIN  TRANS2 ON TRANS1.TRANS1ID=TRANS2.TRANS1ID WHERE TRANS1.T1VCHDT > sysdate-30";
