...
namespace Arasan.Services 
{
    public class PackDrumAllocationService :IPackDrumAllocation
    {
        ...
        public DataTable GetAllPackDerum(string strStatus)
        {
            ...
            {
                SvSql = "select DOCID,to_char(DOCDATE,'dd-MON-yyyy')DOCDATE,PDABASICID,BRANCH,ITEMID,LOCDETAILS.LOCID,PACKLOCID,STOCK ,STARTNO,LASTNO,USERID,ENTEREDBY,SPREFIX,TOTDRUMS from PDABASIC LEFT OUTER JOIN LOCDETAILS ON LOCDETAILSID=PDABASIC.PACKLOCID WHERE PDABASIC.IS_ACTIVE='Y' ORDER BY  PDABASICID DESC";
            }
            else
            {
                SvSql = "select DOCID,to_char(DOCDATE,'dd-MON-yyyy')DOCDATE,PDABASICID,BRANCH,ITEMID,LOCDETAILS.LOCID,PACKLOCID,STOCK ,STARTNO,LASTNO,USERID,ENTEREDBY,SPREFIX,TOTDRUMS from PDABASIC LEFT OUTER JOIN LOCDETAILS ON LOCDETAILSID=PDABASIC.PACKLOCID WHERE PDABASIC.IS_ACTIVE='N' ORDER BY  PDABASICID DESC";
