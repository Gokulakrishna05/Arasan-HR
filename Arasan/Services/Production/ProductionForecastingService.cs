...
namespace Arasan.Services.Production
{
    public class ProductionForecastingService : IProductionForecastingService
    {
        ...
        {
            string SvSql = string.Empty;
            SvSql = "select  PRODFCBASICID,ITEMMASTER.ITEMID,APPOWREQ,APREQ,APAVAILSTK,APMINSTK,APREQQTY,STARTVALUE,ENDVALUE from PRODFCAP LEFT OUTER JOIN  ITEMMASTER ON ITEMMASTER.ITEMMASTERID=PRODFCAP.APITEMID   where PRODFCBASICID= '" + id + "'";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
...
ORDER BY ORD DESC";
        ...
        public List<PFCPYROItem> GetPyroForecast(string mnth, string type)
        {
            ...
            using (OracleConnection con = new OracleConnection(_connectionString))
            {
                ...
                {
                    con.Open();
                    cmd.CommandText = @"SELECT ITEMID,SUM(QTY) REQ,SUM(MINSTK) MINSTK,SUM(stk) stk,SUM(REJ) REJ,Decode(Sign(SUM(QTY+MINSTK-STK)),1,SUM(QTY+MINSTK-STK),0) ORD,STARTVALUE,ENDVALUE FROM (
SELECT ITEMID,SUM(QTY) QTY,SUM(STK) STK,SUM(MINSTK) MINSTK,REJ,STARTVALUE,ENDVALUE FROM (
SELECT I2.ITEMID,0 qty, SUM(DECODE(S.PlusOrMinus,'p',S.qty,-S.qty)) stk,0 MINSTK,i2.REJRAWMATPER REJ,D.STARTVALUE,D.ENDVALUE
FROM StockValue S , ItemMaster I , LocDetails L,ITEMMASTER I2 ,TestTBasic B,TestTDetail D
...
and I.TEMPLATEID=B.TESTTBASICID and B.TESTTBASICID=D.TESTTBASICID and D.TESTDESC='WET SIEVE'
HAVING SUM(DECODE(S.PlusOrMinus,'p',S.qty,-S.qty)) > 0 
GROUP BY I2.ITEMID,i2.REJRAWMATPER,D.STARTVALUE,D.ENDVALUE
UNION ALL
SELECT I.ITEMID,0 qty, Decode(L.RCFLAG,0,SUM(S.PLUSQTY-S.MINUSQTY)) stk,0 MINSTK, 0 rejper,D.STARTVALUE,D.ENDVALUE
FROM LStockValue S , ItemMaster I , LocDetails L,LotMast L ,TestTBasic B,TestTDetail D
...
GROUP BY I.ITEMID,i.REJRAWMATPER,L.RCFLAG,D.STARTVALUE,D.ENDVALUE
Union All
SELECT I2.ITEMID,0 qty, Decode(L.RCFLAG,0,SUM(S.PLUSQTY-S.MINUSQTY)) stk,0 MINSTK, 0 rejper,D.STARTVALUE,D.ENDVALUE
FROM LStockValue S , ItemMaster I , LocDetails L,LotMast L,Itemmaster I2 ,TestTBasic B,TestTDetail D
...
and I.TEMPLATEID=B.TESTTBASICID and B.TESTTBASICID=D.TESTTBASICID and D.TESTDESC='WET SIEVE'
HAVING SUM(S.PLUSQTY-S.MINUSQTY) > 0 
GROUP BY I2.ITEMID,i.REJRAWMATPER,L.RCFLAG,D.STARTVALUE,D.ENDVALUE
UNION ALL
SELECT  I2.ITEMID,SUM(SD.QTY) QTY,0 stk,IM.MINSTK,I2.REJRAWMATPER REJ,D.STARTVALUE,D.ENDVALUE
FROM SALFCBASIC SB,SALFCDETAIL SD,ITEMMASTER IM,ITEMMASTER I2,TestTBasic B,TestTDetail D
WHERE SB.SALFCBASICID=SD.SALFCBASICID
AND SD.ITEMID=IM.ITEMMASTERID
AND IM.ITEMFROM=I2.ITEMMASTERID
AND IM.SUBCATEGORY IN ('PYRO POWDER') 
and IM.TEMPLATEID=B.TESTTBASICID and B.TESTTBASICID=D.TESTTBASICID and D.TESTDESC='WET SIEVE'
AND ((sb.MONTH=:MONTH And SB.FCTYPE=:PlanType) Or (Sb.FCTYPE='YEARLY' And 'YEARLY'=:PlanType))
GROUP BY I2.ITEMID,IM.MINSTK,IM.ITEMID,I2.REJRAWMATPER,D.STARTVALUE,D.ENDVALUE
UNION ALL
SELECT  I1.ITEMID,SUM(SD.QTY*IM.RAWMATPER/100) QTY,0 stk,IM.MINSTK,I1.REJRAWMATPER REJ,D.STARTVALUE,D.ENDVALUE 
FROM SALFCBASIC SB,SALFCDETAIL SD,ITEMMASTER IM,ITEMMASTER I1,ITEMMASTER I2,TestTBasic B,TestTDetail D
WHERE SB.SALFCBASICID=SD.SALFCBASICID
AND SD.ITEMID=IM.ITEMMASTERID
...
AND ((sb.MONTH=:MONTH And  SB.FCTYPE=:PlanType) Or (Sb.FCTYPE='YEARLY' And 'YEARLY'=:PlanType))
GROUP BY I1.ITEMID,IM.MINSTK,I1.REJRAWMATPER,D.STARTVALUE,D.ENDVALUE
UNION ALL
SELECT ITEMID,ROUND(SUM(QTY*RAWP/100),0) QTY,0 STK,0 MINSTK,SUM(REJ) REJ,STARTVALUE,ENDVALUE FROM (
SELECT FG,ITEMID,SUM(QTY+MINSTK-STK) QTY,SUM(REJ) REJ,SUM(RAWP)RAWP,STARTVALUE,ENDVALUE FROM (
SELECT IM.ITEMID FG, I1.ITEMID ,SUM(SD.QTY) QTY,0 stk,IM.MINSTK,I1.REJRAWMATPER REJ,IM.RAWMATPER RAWP,STARTVALUE,ENDVALUE
FROM SALFCBASIC SB,SALFCDETAIL SD,ITEMMASTER IM,ITEMMASTER I1,TestTBasic B,TestTDetail D
WHERE SB.SALFCBASICID=SD.SALFCBASICID
AND SD.ITEMID=IM.ITEMMASTERID
AND IM.ITEMFROM=I1.ITEMMASTERID
...
and IM.TEMPLATEID=B.TESTTBASICID and B.TESTTBASICID=D.TESTTBASICID and D.TESTDESC='WET SIEVE'
AND ((sb.MONTH=:MONTH And  SB.FCTYPE=:PlanType) Or (Sb.FCTYPE='YEARLY' And 'YEARLY'=:PlanType))
GROUP BY IM.ITEMID,I1.ITEMID,IM.MINSTK,I1.REJRAWMATPER,IM.RAWMATPER,IM.MINSTK,STARTVALUE,ENDVALUE
UNION ALL
SELECT I.ITEMID,I2.ITEMID,0 qty, SUM(DECODE(S.PlusOrMinus,'p',S.qty,-S.qty)) stk,0 MINSTK,i2.REJRAWMATPER REJ,0 RAWM,STARTVALUE,ENDVALUE
FROM StockValue S , ItemMaster I , LocDetails L,ITEMMASTER I2,TestTBasic B,TestTDetail D 
WHERE S.ItemID = I.ItemMasterID AND S.DocDate <=:Docdate AND S.LocID = L.LocdetailsID 
...
and I.TEMPLATEID=B.TESTTBASICID and B.TESTTBASICID=D.TESTTBASICID and D.TESTDESC='WET SIEVE'
HAVING SUM(DECODE(S.PlusOrMinus,'p',S.qty,-S.qty)) > 0 
GROUP BY I.ITEMID,I2.ITEMID,i2.REJRAWMATPER,STARTVALUE,ENDVALUE
) GROUP BY ITEMID,FG,STARTVALUE,ENDVALUE
) GROUP BY ITEMID,RAWP,STARTVALUE,ENDVALUE
ORDER BY 2 DESC
...
ORDER BY ord desc";
        ...
        public List<PFCPASTEItem> GetPasteForecast(string mnth, string type)
        {
            ...
            using (OracleConnection con = new OracleConnection(_connectionString))
            {
                ...
                {
                    con.Open();
                    cmd.CommandText = @"Select Itemid,Decode(sign(Sum(qty-stk+min)),1,Sum(qty-stk+min) ,0) qty,Sum(qty) tar,Sum(stk) stk,Sum(min) min,STARTVALUE,ENDVALUE from (
SELECT I2.ITEMID,SUM(SD.QTY*0.9) QTY,0 stk, 0 min ,STARTVALUE,ENDVALUE
FROM SALFCBASIC SB,SALFCDETAIL SD,ITEMMASTER IM,ITEMMASTER I2,TestTBasic B,TestTDetail D
WHERE SB.SALFCBASICID=SD.SALFCBASICID
AND SD.ITEMID=IM.ITEMMASTERID
AND IM.ITEMFROM=I2.ITEMMASTERID
and IM.TEMPLATEID=B.TESTTBASICID and B.TESTTBASICID=D.TESTTBASICID and D.TESTDESC='LEAFING'
AND IM.SUBCATEGORY IN ('NON LEAFING PASTE','LEAFING PASTE') 
AND ((sb.MONTH=:MONTH And  SB.FCTYPE=:PlanType) Or (Sb.FCTYPE='YEARLY' And 'YEARLY'=:Plantype))
GROUP BY SB.DOCID,I2.ITEMID,STARTVALUE,ENDVALUE
Union ALL
Select I1.itemid,0,Sum(x.stk) stk,Sum(x.minstk) minstk,STARTVALUE,ENDVALUE From (
SELECT I.ITEMID,0,SUM(DECODE(S.PlusOrMinus,'p',S.qty,-S.qty)) stk,I.MINSTK,I.ITEmFrom iid,STARTVALUE,ENDVALUE
...
and I.TEMPLATEID=B.TESTTBASICID and B.TESTTBASICID=D.TESTTBASICID and D.TESTDESC='LEAFING'
HAVING SUM(DECODE(S.PlusOrMinus,'p',S.qty,-S.qty)) > 0 
GROUP BY I.ITEMID,I.MINSTK,I.ITEmFrom,STARTVALUE,ENDVALUE
) X,Itemmaster I1 where I1.itemMasterID=X.iid
Group By I1.itemid,STARTVALUE,ENDVALUE
Union ALL
SELECT I.ITEMID,0, SUM(DECODE(S.PlusOrMinus,'p',S.qty,-S.qty)) stk,I.MINSTK,STARTVALUE,ENDVALUE
FROM StockValue S , ItemMaster I , LocDetails L,TestTBasic B,TestTDetail D   
WHERE S.ItemID = I.ItemMasterID AND S.DocDate <=:Docdate AND S.LocID = L.LocdetailsID 
...
and I.TEMPLATEID=B.TESTTBASICID and B.TESTTBASICID=D.TESTTBASICID and D.TESTDESC='LEAFING'
Group by I.ITEMID,STARTVALUE,ENDVALUE
Order by 2 Desc";
                    cmd.Parameters.Add("Docdate", Docdate);
                    cmd.Parameters.Add("PlanType", type);
                    cmd.Parameters.Add("MONTH", mnth);
                ...
                using (OracleConnection objConn = new OracleConnection(_connectionString))
                {
                    ...
                    try
                    {
                        ...
                        if (cy.Aplst != null)
                        {
                            if (cy.ID == null)
                            {
                                foreach (ProdApItem cp in cy.Aplst)
                                {
                                    if (cp.saveitemid != null)
                                    {
                                        svSQL = "Insert into PRODFCAP (PRODFCBASICID,APPOWREQ,APREQ,APAVAILSTK,APMINSTK,APITEMID,APREQQTY,STARTVALUE,ENDVALUE) VALUES ('" + Pid + "','" + cp.reqappowder + "','" + cp.reqqty + "','" + cp.avlstk + "','" + cp.ministk + "','" + cp.saveitemid + "','" + cp.ordqty + "','" + cp.startvalue + "','" + cp.endvalue + "')";
                                        OracleCommand objCmds = new OracleCommand(svSQL, objConn);
                                        objCmds.ExecuteNonQuery();
                                    }
