...
namespace Arasan.Services.Production
{
    public class ProductionScheduleService : IProductionScheduleService
    {
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select BRANCHMAST.BRANCHID,SCHPLANTYPE,DOCID,to_char(PSBASIC.DOCDATE,'dd-MON-yyyy')DOCDATE,to_char(PSBASIC.FROMDATE,'dd-MON-yyyy')FROMDATE,to_char(PSBASIC.TODATE,'dd-MON-yyyy')TODATE,WCBASIC.WCID,PROCESSMAST.PROCESSID,to_char(PSBASIC.SCHDATE,'dd-MON-yyyy') SCHDATE,OPQTY,PRODQTY,FORMULA,to_char(PSBASIC.PDOCDT,'dd-MON-yyyy') PDOCDT,ITEMMASTER.ITEMID,OPUNIT,EXPRUNHRS,REFSCHNO,AMDSCHNO,ENTEREDBY from PSBASIC LEFT OUTER JOIN BRANCHMAST ON BRANCHMASTID=PSBASIC.BRANCHID LEFT OUTER JOIN WCBASIC ON PSBASIC.WCID=WCBASIC.WCBASICID LEFT OUTER JOIN PROCESSMAST ON PROCESSMAST.PROCESSMASTID=PSBASIC.PROCESSID LEFT OUTER JOIN ITEMMASTER ON ITEMMASTERID=PSBASIC.OPITEMID Where PSBASICID='" + PROID + "'";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        {
            string SvSql = string.Empty;
            SvSql = "select PSBASICID,ITEMMASTER.ITEMID,PKITEMID ,PKQTY from PSPACKDETAIL LEFT OUTER JOIN ITEMMASTER ON ITEMMASTER.ITEMMASTERID=PSPACKDETAIL.PKITEMID   where PSBASICID='" + PROID + "' ";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
