...
namespace Arasan.Services
{
    public class PurchaseImportQuoService : IPurchaseImportQuo
    {
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select  BRANCHMAST.BRANCHID, DOCID,to_char(DOCDATE,'dd-MON-yyyy') DOCDATE ,PARTYMAST.PARTYNAME,ENQNO,to_char(ENQDATE,'dd-MON-yyyy') ENQDATE,MAINCURRENCY,IPURQUOTBASICID,IPURQUOTBASIC.STATUS from IPURQUOTBASIC LEFT OUTER JOIN BRANCHMAST ON BRANCHMASTID=IPURQUOTBASIC.BRANCHID LEFT OUTER JOIN  PARTYMAST on IPURQUOTBASIC.PARTYID=PARTYMAST.PARTYMASTID Where PARTYMAST.TYPE IN ('Supplier','BOTH') AND IPURQUOTBASICID='" + name + "'";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        public IEnumerable<QuoImportFollowup> GetAllPurchaseFollowup()
        {
            List<QuoImportFollowup> cmpList = new List<QuoImportFollowup>();
            using (OracleConnection con = new OracleConnection(_connectionString))
            {
        ...
        public string PurchaseFollowupCRUD(QuoImportFollowup cy)
        {
            string msg = "";
            try
            {
        ...
        public async Task<IEnumerable<PQuoItemDetail>> GetPQuoItem(string id)
        {
            ...
            {
                return await db.QueryAsync<PQuoItemDetail>("SELECT  IPURQUOTDETAIL.IPURQUOTBASICID,IPURQUOTBASIC.IPURQUOTBASICID,TAAIERP.IPURQUOTBASIC.DOCID, TAAIERP.IPURQUOTBASIC.DOCDATE, TAAIERP.IPURQUOTBASIC.ENQDATE, TAAIERP.ITEMMASTER.ITEMID, TAAIERP.IPURQUOTDETAIL.UNIT, TAAIERP.IPURQUOTDETAIL.CF, TAAIERP.IPURQUOTDETAIL.QTY, TAAIERP.IPURQUOTDETAIL.RATE, TAAIERP.IPURQUOTDETAIL.PRIQTY, TAAIERP.IPURQUOTDETAIL.ITEMDESC, TAAIERP.PARTYMAST.PARTYNAME, TAAIERP.PARTYMAST.PARTYID, TAAIERP.PARTYMAST.ADD1, TAAIERP.PARTYMAST.ADD2, TAAIERP.PARTYMAST.ADD3, TAAIERP.PARTYMAST.STATE, TAAIERP.PARTYMAST.GSTNO FROM    TAAIERP.PARTYMAST INNER JOIN  TAAIERP.IPURQUOTBASIC ON TAAIERP.PARTYMAST.PARTYMASTID = TAAIERP.IPURQUOTBASIC.PARTYID INNER JOIN TAAIERP.IPURQUOTDETAIL ON TAAIERP.IPURQUOTBASIC.IPURQUOTBASICID=TAAIERP.IPURQUOTDETAIL.IPURQUOTBASICID LEFT OUTER JOIN ITEMMASTER ON ITEMMASTER.ITEMMASTERID=IPURQUOTDETAIL.ITEMID where IPURQUOTDETAIL.IPURQUOTBASICID='" + id + "' and IPURQUOTBASIC.IPURQUOTBASICID ='" + id + "'", commandType: CommandType.Text);
