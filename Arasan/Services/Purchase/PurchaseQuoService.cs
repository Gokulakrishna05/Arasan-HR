...
namespace Arasan.Services
{
    public class PurchaseQuoService : IPurchaseQuo
    {
        ...
        public DataTable GetAllPurchaseQuoItems(string strStatus)
        {
            ...
            {
                SvSql = "Select  B.BRANCHID, Q.DOCID,to_char(Q.DOCDATE,'dd-MON-yyyy') DOCDATE ,P.PARTYNAME,E.DOCID ENQNO,E.DOCDATE ENQDATE,Q.MAINCURRENCY,Q.EXRATE,PURQUOTBASICID,Q.STATUS from PURQUOTBASIC Q,PURENQBASIC  E,BRANCHMAST B,PARTYMAST P Where E.PURENQBASICID=Q.ENQNO  AND BRANCHMASTID=Q.BRANCHID AND Q.PARTYID=P.PARTYMASTID AND  E.IS_ACTIVE='Y' ORDER BY Q.PURQUOTBASICID DESC";
            }
            else
            {
                SvSql = "Select  B.BRANCHID, Q.DOCID,to_char(Q.DOCDATE,'dd-MON-yyyy') DOCDATE ,P.PARTYNAME,E.DOCID ENQNO,E.DOCDATE ENQDATE,Q.MAINCURRENCY,Q.EXRATE,PURQUOTBASICID,Q.STATUS from PURQUOTBASIC Q,PURENQBASIC  E,BRANCHMAST B,PARTYMAST P Where E.PURENQBASICID=Q.ENQNO  AND BRANCHMASTID=Q.BRANCHID AND Q.PARTYID=P.PARTYMASTID AND  E.IS_ACTIVE='N' ORDER BY Q.PURQUOTBASICID DESC";
            }
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
