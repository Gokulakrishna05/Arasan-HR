...
namespace Arasan.Services.Qualitycontrol
{
    public class QCResultService : IQCResultService
    {
        ...
        public DataTable GetQCResultGrid(string strStatus)
        {
            string SvSql = string.Empty;
            if (strStatus == "Y" || strStatus == null)
            {
                SvSql = "Select GRNBLBASIC.DOCID as GRN,QCRESULTBASIC.DOCID,to_char(QCRESULTBASIC.DOCDATE,'dd-MON-yyyy')DOCDATE,to_char(QCRESULTBASIC.GRNDATE,'dd-MON-yyyy')GRNDATE,PARTYMAST.PARTYNAME,QCRESULTBASIC.TESTEDBY,LOCDETAILS.LOCID,QCRESULTBASIC.REMARKS,QCRESULTBASIC.QCLOCATION,QCRESULTBASICID from QCRESULTBASIC LEFT OUTER JOIN LOCDETAILS ON LOCDETAILS.LOCDETAILSID=QCRESULTBASIC.LOCATION LEFT OUTER JOIN  PARTYMAST on QCRESULTBASIC.PARTYID=PARTYMAST.PARTYMASTID LEFT OUTER JOIN  GRNBLBASIC on GRNBLBASIC.GRNBLBASICID=QCRESULTBASIC.GRNNO   WHERE QCRESULTBASIC.IS_ACTIVE='Y' order by QCRESULTBASICID desc";
            }
            else
            {
                SvSql = "Select GRNBLBASIC.DOCID as GRN,QCRESULTBASIC.DOCID,to_char(QCRESULTBASIC.DOCDATE,'dd-MON-yyyy')DOCDATE,to_char(QCRESULTBASIC.GRNDATE,'dd-MON-yyyy')GRNDATE,PARTYMAST.PARTYNAME,QCRESULTBASIC.TESTEDBY,LOCDETAILS.LOCID,QCRESULTBASIC.REMARKS,QCRESULTBASIC.QCLOCATION,QCRESULTBASICID from QCRESULTBASIC LEFT OUTER JOIN LOCDETAILS ON LOCDETAILS.LOCDETAILSID=QCRESULTBASIC.LOCATION LEFT OUTER JOIN  PARTYMAST on QCRESULTBASIC.PARTYID=PARTYMAST.PARTYMASTID LEFT OUTER JOIN  GRNBLBASIC on GRNBLBASIC.GRNBLBASICID=QCRESULTBASIC.GRNNO   WHERE QCRESULTBASIC.IS_ACTIVE='N' order by QCRESULTBASICID desc";
            }
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
