...
namespace Arasan.Services.Qualitycontrol
{
    public class QCTestValueEntryService : IQCTestValueEntryService
    {
        ...
        {
            string SvSql = string.Empty;
            SvSql = "select WCID,SHIFT,to_char(BPRODBASIC.DOCDATE,'dd-MON-yyyy')DOCDATE from BPRODBASIC WHERE BPRODBASICID='" + id + "' ";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        {
            string SvSql = string.Empty;
            SvSql = "select ITEMMASTER.ITEMID,BPRODOUTDET.OITEMID ,OCDRUMNO,BPRODBASICID,BPRODOUTDETID from BPRODOUTDET  LEFT OUTER JOIN ITEMMASTER ON ITEMMASTER.ITEMMASTERID=BPRODOUTDET.OITEMID   WHERE BPRODOUTDETID='" + id + "' ";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        {
            string SvSql = string.Empty;
            SvSql = "select SHIFTMAST.SHIFTNO,WCBASIC.WCID,BRANCHMAST.BRANCHID,DOCID,to_char(QTVEBASIC.DOCDATE,'dd-MON-yyyy')DOCDATE,QTVEBASIC.PROCESSLOTNO,QTVEBASIC.CDRUMNO,to_char(QTVEBASIC.DOCDATE,'dd-MON-yyyy')PRODDATE,QTVEBASIC.SAMPLENO,QTVEBASIC.STIME,QTVEBASIC.ITEMID,QTVEBASIC.RATEPHR,QTVEBASIC.NOZZLENO,QTVEBASIC.AIRPRESS,QTVEBASIC.ADDCH,QTVEBASIC.BCT,QTVEBASIC.ENTEREDBY,QTVEBASIC.REMARKS,QTVEBASIC.QTVEBASICID from QTVEBASIC LEFT OUTER JOIN BRANCHMAST ON BRANCHMASTID = QTVEBASIC.BRANCH LEFT OUTER JOIN WCBASIC ON WCBASICID = QTVEBASIC.WCID LEFT OUTER JOIN SHIFTMAST ON SHIFTMASTID = QTVEBASIC.SHIFTNO WHERE QTVEBASICID='" + id + "'";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
