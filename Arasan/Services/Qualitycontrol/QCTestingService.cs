    ...
    public class QCTestingService : IQCTestingService
    {
        ...
        public string QCTestingCRUD(QCTesting cy)
        {
            ...
            try
            {
                ...
                DataTable itemma = datatrans.GetData("SELECT LOTYN,SERIALYN FROM ITEMMASTER WHERE ITEMMASTERID='" + cy.Item + "'");
                ...
                using ( OracleConnection objConn = new OracleConnection(_connectionString))
                 {
                    ...
                    objCmd.Parameters.Add("CLASSCODE", OracleDbType.NVarchar2).Value = "A";
                    ...
                    objCmd.Parameters.Add("TEMPLATEID", OracleDbType.NVarchar2).Value = cy.temp;
                    objCmd.Parameters.Add("GRNQTY", OracleDbType.NVarchar2).Value = cy.GRNProd;
                    objCmd.Parameters.Add("LOTYN", OracleDbType.NVarchar2).Value = itemma.Rows[0]["LOTYN"].ToString();
                    objCmd.Parameters.Add("SERIALYN", OracleDbType.NVarchar2).Value = itemma.Rows[0]["SERIALYN"].ToString();
                        ...
                        foreach (QCGRNItem cp in cy.QCGRNLst)
                        {
                            if (cp.Isvalid == "Y" && cp.testid != "0")
                            {
                                ...
                                using (OracleConnection objConns = new OracleConnection(_connectionString))
                                {
                                    ...
                                    svSQL = "Update GRNBLDETAIL set QCTESTFLAG ='1' WHERE  GRNBLDETAILID='" + cy.detid + "'";
                                    ...
                                    objCmdd.ExecuteNonQuery();
                                    svSQL = "Update LOTMAST set INSFLAG ='1' WHERE  T1SOURCEID='" + cy.detid + "'";
                                    objCmdd = new OracleCommand(svSQL, objConn);
                                    objCmdd.ExecuteNonQuery();
                                    svSQL = "Update STOCKVALUE set SINSFLAG ='1' WHERE  T1SOURCEID='" + cy.APID + "'";
                                    objCmdd = new OracleCommand(svSQL, objConn);
                                    objCmdd.ExecuteNonQuery();
                                    objConns.Close();
                                }
        ...
        public string POQCTestingCRUD(QCTesting cy)
        {
            ...
            try
            {
                ...
                using (OracleConnection objConn = new OracleConnection(_connectionString))
                {
                    ...
                    objCmd.Parameters.Add("GRNPROD", OracleDbType.NVarchar2).Value = cy.Qty;
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select ITEMMASTER.ITEMID,GRNBLDETAIL.ITEMID ,GRNBLBASICID,GRNBLDETAILID from GRNBLDETAIL LEFT OUTER JOIN ITEMMASTER ON ITEMMASTERID=GRNBLDETAIL.ITEMID where GRNBLDETAIL.GRNBLBASICID='" + id + "'";
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select GRNBLBASIC.DOCID,PARTYMAST.PARTYID ,ITEMMASTER.ITEMID,QCVALUEBASIC.DOCID,to_char(QCVALUEBASIC.DOCDATE,'dd-MON-yyyy') DOCDATE,to_char(QCVALUEBASIC.GRNDATE,'dd-MON-yyyy')GRNDATE,QCVALUEBASIC.CLASSCODE,QCVALUEBASICID,QCVALUEBASIC.LOTSERIALNO,QCVALUEBASIC.SLNO,QCVALUEBASIC.TESTRESULT,QCVALUEBASIC.TESTBY,QCVALUEBASIC.REMARKS ,QCVALUEBASIC.GRNPROD ,QCVALUEBASIC.TESTPROCEDURE from QCVALUEBASIC LEFT OUTER JOIN  PARTYMAST on PARTYMAST.PARTYMASTID = QCVALUEBASIC.PARTYID LEFT OUTER JOIN  GRNBLBASIC on GRNBLBASIC.GRNBLBASICID = QCVALUEBASIC.GRNNO LEFT OUTER JOIN  ITEMMASTER on ITEMMASTER.ITEMMASTERID = QCVALUEBASIC.ITEMID Where QCVALUEBASIC.QCVALUEBASICID='" + id + "' ";
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select TESTTDETAIL.TESTDESC,QCVALUEDETAIL.ACVAL,QCVALUEDETAIL.TESTVALUE,QCVALUEDETAIL.RESULT,QCVALUEDETAIL.MANUALVALUE,QCVALUEDETAIL.ACTTESTVALUE from QCVALUEDETAIL LEFT OUTER JOIN  TESTTDETAIL on QCVALUEDETAIL.TESTDESC=TESTTDETAIL.TESTTDETAILID Where QCVALUEBASICID ='" + id + "'";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        {
            string SvSql = string.Empty;
            SvSql = "select ITEMMASTER.ITEMID,GRNBLDETAILID,GRNBLDETAIL.ITEMID as item,QTY,GRNBLDETAILID,GRNBLBASICID from GRNBLDETAIL  LEFT OUTER JOIN ITEMMASTER ON ITEMMASTER.ITEMMASTERID=GRNBLDETAIL.ITEMID   WHERE GRNBLDETAILID='" + id + "' ";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
