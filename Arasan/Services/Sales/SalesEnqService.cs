    ...
    public class SalesEnqService : ISalesEnq
    {
                ...
                using (OracleCommand cmd = con.CreateCommand())
                {
                    ...
                    while (rdr.Read())
                    {
                        SalesEnquiry cmp = new SalesEnquiry 
                        {
                            ...
                            ID = rdr["SALESENQUIRYID"].ToString(),
        ...
        public IEnumerable<SalesItem> GetAllSalesenquriyItem(string id)
        {
            ...
            using (OracleConnection con = new OracleConnection(_connectionString))
            {
                ...
                {
                    con.Open();
                    cmd.CommandText = "Select SALES_ENQ_ITEM.SALESENQITEMID,SALES_ENQ_ITEM.ITEM_DESCRIPTION,SALES_ENQ_ITEM.QUANTITY,ITEMMASTER.ITEMID,UNITMAST.UNITID from SALES_ENQ_ITEM LEFT OUTER JOIN ITEMMASTER on ITEMMASTER.ITEMMASTERID=SALES_ENQ_ITEM.ITEM_ID LEFT OUTER JOIN UNITMAST ON UNITMAST.UNITMASTID=ITEMMASTER.PRIUNIT  where SALES_ENQ_ITEM.SAL_ENQ_ID='" + id + "'";
                    OracleDataReader rdr = cmd.ExecuteReader();
                    while (rdr.Read())
                    {
                        SalesItem cmp = new SalesItem
                        {
                            ID = rdr["SALESENQITEMID"].ToString(),
                ...
                using (OracleConnection objConn = new OracleConnection(_connectionString))
                 {
                    ...
                    objCmd.Parameters.Add("ENQ_DATE", OracleDbType.NVarchar2).Value = cy.EnqDate;
        ...
        public string EnquirytoQuote(string QuoteId)
        {
            ...
            try
            {
                ...
                string QUONo = string.Format("{0}{1}", "SQUO#", (idc + 1).ToString());
                ...
                using (OracleConnection objConn = new OracleConnection(_connectionString))
                {
                    svSQL = "Insert into SALES_QUOTE (BRANCHID,ENQ_NO,ENQ_DATE,SALES_ENQ_ID,CURRENCY_TYPE,QUOTE_NO,QUOTE_DATE,CONTACT_PERSON,CONTACT_PERSON_MOBILE,PRIORITY,ADDRESS,CITY,CUSTOMER_TYPE,CUSTOMER,PINCODE,QUOTETYPE,IS_ACTIVE) (Select BRANCH_ID,ENQ_NO,ENQ_DATE,'" + QuoteId + "',CURRENCY_TYPE,'" + QUONo + "','" + DateTime.Now.ToString("dd-MMM-yyyy") + "' ,CONTACT_PERSON,CONTACT_PERSON_MOBILE,PRIORITY,ADDRESS,CITY,CUSTOMER_TYPE,CUSTOMER_NAME,PINCODE,ENQ_TYPE,'Y' from SALES_ENQUIRY where SALES_ENQUIRY.SALESENQUIRYID='" + QuoteId + "')";
                ...
                {
                    string Sql = "Insert into SALESQUOTEDETAIL (SALESQUOTEID,ITEMID,ITEMDESC,QTY,UNIT,AMOUNT,RATE) (Select '" + quotid + "',ITEM_ID,ITEM_DESCRIPTION,QUANTITY,UNIT,AMOUNT,RATE FROM SALES_ENQ_ITEM WHERE SAL_ENQ_ID=" + QuoteId + ")";
                ...
                {
                    string Sql = "UPDATE SALES_ENQUIRY SET STATUS='Generated' where SALES_ENQUIRY.SALESENQUIRYID='" + QuoteId + "'";
        ...
        public DataTable GetSalesEnquiry(string id)
        {
            ...
            SvSql = "Select  BRANCH_ID,SALES_ENQUIRY.ENQ_NO,to_char(ENQ_DATE,'dd-MON-yyyy')ENQ_DATE, ENQ_TYPE,CUSTOMER_TYPE,CUSTOMER_NAME,ADDRESS,CITY,PINCODE,CONTACT_PERSON,CONTACT_PERSON_MOBILE,PRIORITY,LEADBY,ASSIGNED_TO,CURRENCY_TYPE,SALES_ENQUIRY.SALESENQUIRYID from SALES_ENQUIRY  where SALES_ENQUIRY.SALESENQUIRYID=" + id + "";
        ...
        public DataTable GetSalesEnquiryItem(string id)
        {
            ...
            SvSql = "Select ITEMMASTER.ITEMID,ITEM_DESCRIPTION,UNIT,QUANTITY,RATE,AMOUNT  from SALES_ENQ_ITEM LEFT OUTER JOIN ITEMMASTER ON  ITEMMASTER.ITEMMASTERID=SALES_ENQ_ITEM.ITEM_ID  where SALES_ENQ_ITEM.SAL_ENQ_ID=" + id + " ";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select ITEMMASTER.ITEMID,ITEM_DESCRIPTION,UNIT,QUANTITY,AMOUNT,RATE  from SALES_ENQ_ITEM LEFT OUTER JOIN ITEMMASTER ON  ITEMMASTER.ITEMMASTERID=SALES_ENQ_ITEM.ITEM_ID  where SAL_ENQ_ID=" + name + " ";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
