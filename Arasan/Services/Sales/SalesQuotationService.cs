...
namespace Arasan.Services.Sales
{
    public class SalesQuotationService : ISalesQuotationService
    {
        ...
        public IEnumerable<SalesQuotation> GetAllSalesQuotation(string status)
        {
            ...
            using (OracleConnection con = new OracleConnection(_connectionString))
            {
                ...
                using (OracleCommand cmd = con.CreateCommand())
                {
                    ...
                    cmd.CommandText = "Select SALESQUOTEID,NEW_EXISTINGQUOTE,FROM_ENQUIRY,BRANCHID,QUOTE_NO,to_char(QUOTE_DATE,'dd-MON-yyyy')QUOTE_DATE,QUOTETYPE,SQ.CURRENCY_TYPE,SQ.CUSTOMER,SQ.CUSTOMER_TYPE,SQ.ADDRESS,SQ.CITY,SQ.CONTACT_PERSON_MOBILE,SQ.CONTACT_PERSON_MAIL,SQ.PINCODE,SQ.PRIORITY,COUNTRYSHIPMENT,SQ.ASSIGNED_TO,SQ.SALESQUOTEID,SQ.STATUS ,SQ.GROSS,SQ.NET from SALES_QUOTE SQ LEFT OUTER JOIN  PARTYMAST on SQ.CUSTOMER=PARTYMAST.PARTYMASTID   ";
                    ...
                    while (rdr.Read())
                    {
                        SalesQuotation cmp = new SalesQuotation
                        {
                            ...
                            QuoType = rdr["QUOTETYPE"].ToString(),
                            ...
                            Country = rdr["COUNTRYSHIPMENT"].ToString(),
                            Assign = rdr["ASSIGNED_TO"].ToString(),
                            
                            status = rdr["STATUS"].ToString(),
                            QuoteFormatId = rdr["NEW_EXISTINGQUOTE"].ToString(),
                            EnquiryId = rdr["FROM_ENQUIRY"].ToString(),
        ...
        public string SalesQuotationCRUD(SalesQuotation cy)
        {
            ...
            try
            {
                ...
                using (OracleConnection objConn = new OracleConnection(_connectionString))
                {
                    ...
                    objCmd.Parameters.Add("QUOTETYPE", OracleDbType.NVarchar2).Value = cy.EnqType;
                    ...
                    objCmd.Parameters.Add("COUNTRYSHIPMENT", OracleDbType.NVarchar2).Value = cy.Country;
                    ...
                    objCmd.Parameters.Add("FROM_ENQUIRY", OracleDbType.NVarchar2).Value = cy.EnquiryId;
                        ...
                        foreach (QuoItem cp in cy.QuoLst)
                        {
                            if (cp.Isvalid == "Y" && cp.itemid != "0")
                            {
                                using (OracleConnection objConns = new OracleConnection(_connectionString))
                                {
                                    ...
                                    objCmds.Parameters.Add("SALESQUOTEID", OracleDbType.NVarchar2).Value = Pid;
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select SALES_QUOTE.BRANCHID,NEW_EXISTINGQUOTE,FROM_ENQUIRY,SALES_QUOTE.QUOTE_NO,to_char(SALES_QUOTE.QUOTE_DATE,'dd-MON-yyyy')QUOTE_DATE,QUOTETYPE,SALES_QUOTE.CURRENCY_TYPE,SALES_QUOTE.CUSTOMER,SALES_QUOTE.CUSTOMER_TYPE,SALES_QUOTE.ADDRESS,SALES_QUOTE.CITY,SALES_QUOTE.PINCODE,SALES_QUOTE.CONTACT_PERSON_MAIL,SALES_QUOTE.CONTACT_PERSON_MOBILE,SALES_QUOTE.PRIORITY,COUNTRYSHIPMENT,SALES_QUOTE.ASSIGNED_TO,SALESQUOTEID,SALES_QUOTE.GROSS,SALES_QUOTE.NET,SALES_QUOTE.COUNTRYSHIPMENT,SALES_QUOTE.ENQ_NO from SALES_QUOTE LEFT OUTER JOIN  SALES_ENQUIRY ON SALES_ENQUIRY.SALESENQUIRYID=SALES_QUOTE.SALESQUOTEID where SALES_QUOTE.SALESQUOTEID= '" + id + "' ";
        ...
        public DataTable GetSalesQuotationItemDetails(string id)
        {
            ...
            SvSql = "SELECT ITEMID,ITEMDESC,RATE,QTY,CF,UNIT,AMOUNT,DISC,DISCAMOUNT,IFREIGHTCH,TOTAMT,CGSTPER,SGSTPER,IGSTPER,SGSTAMT,IGSTAMT,CGSTAMT,SALESQUOTEID FROM SALESQUOTEDETAIL WHERE SALESQUOTEDETAIL.SALESQUOTEID ='" + id + "'";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        public string SalesQuotationWorkOrder(string QuoId)
        {
            string msg = "";
            try
            {
                ...
                string WorkNo = string.Format("{0}{1}", "JOB#", (idc + 1).ToString());
                ...
                using (OracleConnection objConn = new OracleConnection(_connectionString))
                {
                    svSQL = "Insert into JOBASIC (PARTYID,BRANCHID,QUOTNO,DOCID,DOCDATE,IS_ACTIVE) (Select CUSTOMER,BRANCHID,'" + QuoId + "','" + WorkNo + "','" + DateTime.Now.ToString("dd-MMM-yyyy") + "','Y' from SALES_QUOTE where SALESQUOTEID='" + QuoId + "')";
                ...
                string quotid = datatrans.GetDataString("Select JOBASICID from JOBASIC Where QUOTNO=" + QuoId + "");
                using (OracleConnection objConnT = new OracleConnection(_connectionString))
                {
                    string Sql = "Insert into JODETAIL (JOBASICID,ITEMID,RATE,QTY,UNIT) (Select '" + quotid + "',ITEMID,RATE,QTY,UNIT FROM SALESQUOTEDETAIL WHERE SALESQUOTEDETAILID=" + QuoId + ")";
                ...
                {
                    string Sql = "UPDATE SALES_QUOTE SET STATUS='Generated' where SALESQUOTEID='" + QuoId + "'";
       ...
       public async Task<IEnumerable<SQuoItemDetail>> GetSQuoItem(string id)
        {
            using (OracleConnection db = new OracleConnection(_connectionString))
            {
                ...
                return await db.QueryAsync<SQuoItemDetail>(" SELECT TAAIERP.PARTYMAST.PARTYNAME, TAAIERP.PARTYMAST.ADD1, TAAIERP.PARTYMAST.ADD2, TAAIERP.PARTYMAST.ADD3, TAAIERP.PARTYMAST.STATE, TAAIERP.PARTYMAST.CITY, TAAIERP.PARTYMAST.PINCODE, TAAIERP.PARTYMAST.GSTNO, TAAIERP.SALES_QUOTE.QUOTE_NO, TAAIERP.SALES_QUOTE.QUOTE_DATE, TAAIERP.SALES_QUOTE.DELIVERY_TERMS,TAAIERP.SALES_QUOTE.AMTINWORDS, TAAIERP.SALES_QUOTE.TOTAL_PRICE, TAAIERP.SALES_QUOTE.TOTAL_QUANTIRY, TAAIERP.SALES_QUOTE.GROSS,TAAIERP.SALES_QUOTE.NET,TAAIERP.SALESQUOTEDETAIL.ITEMID, TAAIERP.SALESQUOTEDETAIL.RATE, TAAIERP.SALESQUOTEDETAIL.QTY, TAAIERP.SALESQUOTEDETAIL.UNIT, TAAIERP.SALESQUOTEDETAIL.AMOUNT, TAAIERP.SALESQUOTEDETAIL.TOTAMT, TAAIERP.SALESQUOTEDETAIL.SGSTAMT, TAAIERP.SALESQUOTEDETAIL.IGSTAMT, TAAIERP.SALESQUOTEDETAIL.CGSTAMT FROM PARTYMAST INNER JOIN SALES_QUOTE ON PARTYMAST.PARTYMASTID = SALES_QUOTE.SALESQUOID INNER JOIN SALESQUOTEDETAIL ON SALES_QUOTE.SALESQUOTEID = SALESQUOTEDETAIL.SALESQUOTEDETAILID WHERE SALESQUOTEDETAIL.SALESQUOTEDETAILID ='" + id + "' and SALES_QUOTE.SALESQUOTEID  ='" + id + "' ", commandType: CommandType.Text);
        ...
        public DataTable GetAllListSalesQuotationItems(string strStatus)
        {
            ...
            {
                SvSql = "SELECT SALESQUOTEID,QUOTE_NO,to_char(SALES_QUOTE.QUOTE_DATE,'dd-MON-yyyy')QUOTE_DATE,ENQ_NO,PARTYMAST.PARTYID,SALES_QUOTE.STATUS FROM SALES_QUOTE LEFT OUTER JOIN PARTYMAST ON PARTYMASTID=SALES_QUOTE.CUSTOMER WHERE SALES_QUOTE.IS_ACTIVE='Y' ORDER BY SALES_QUOTE.SALESQUOTEID DESC";
            }
            else
            {
                SvSql = "SELECT SALESQUOTEID,QUOTE_NO,to_char(SALES_QUOTE.QUOTE_DATE,'dd-MON-yyyy')QUOTE_DATE,ENQ_NO,PARTYMAST.PARTYID,SALES_QUOTE.STATUS FROM SALES_QUOTE LEFT OUTER JOIN PARTYMAST ON PARTYMASTID=SALES_QUOTE.CUSTOMER WHERE SALES_QUOTE.IS_ACTIVE='N' ORDER BY SALES_QUOTE.SALESQUOTEID DESC";
            }
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
        ...
        public DataTable GetSalesQuotationByName(string id)
        {
            string SvSql = string.Empty;
            SvSql = "SELECT SALESQUOTEID,BRANCHMAST.BRANCHID,QUOTE_NO,to_char(SALES_QUOTE.QUOTE_DATE,'dd-MON-yyyy')QUOTE_DATE,ENQ_NO,to_char(SALES_QUOTE.ENQ_DATE,'dd-MON-yyyy')ENQ_DATE,PARTYMAST.PARTYNAME FROM SALES_QUOTE LEFT OUTER JOIN  PARTYMAST on PARTYMAST.PARTYMASTID=SALES_QUOTE.CUSTOMER LEFT OUTER JOIN BRANCHMAST ON BRANCHMAST.BRANCHMASTID=SALES_QUOTE.BRANCHID  WHERE SALES_QUOTE.SALESQUOTEID=" + id + "";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        {
            string SvSql = string.Empty;
            SvSql = "SELECT SALESQUOTEID,ITEMMASTER.ITEMID,RATE,QTY,UNIT,AMOUNT,TOTAMT,SALESQUOTEDETAIL.SALESQUOTEDETAILID FROM SALESQUOTEDETAIL LEFT OUTER JOIN  ITEMMASTER ON ITEMMASTERID=SALESQUOTEDETAIL.ITEMID\r\n WHERE SALESQUOTEID=" + id + "";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
