...
namespace Arasan.Services.Sales
{
    public class SalesQuotationService : ISalesQuotationService
    {
        ...
        public DataTable GetEnqDetails(string id)
        {
            string SvSql = string.Empty;
            //SvSql = "Select CITY,PINCODE,ADD1,ADD2,ADD3,INTRODUCEDBY from PARTYMAST Where PARTYMAST.PARTYMASTID='" + id + "' ";
            SvSql = "select ENQ_NO,ENQ_TYPE,ENQ_DATE,CURRENCY_TYPE,CUSTOMER_NAME,SALESENQUIRYID from SALES_ENQUIRY WHERE SALES_ENQUIRY.SALESENQUIRYID = '" + id + "' ";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select SALES_QUOTE.SALESQUOTEID,CURRENCY_TYPE,SALESENQUIRYID from SALES_ENQUIRY LEFT OUTER JOIN SALES_QUOTE ON SALESQUOTEID=SALES_ENQUIRY.SALESENQUIRYID WHERE SALES_ENQUIRY.SALESENQUIRYID='" + id + "'";
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select SALES_QUOTE.SALESQUOTEID,CUSTOMER_NAME,SALESENQUIRYID from SALES_ENQUIRY LEFT OUTER JOIN SALES_QUOTE ON SALESQUOTEID=SALES_ENQUIRY.SALESENQUIRYID WHERE SALES_ENQUIRY.SALESENQUIRYID='" + id + "'";
        ...
        public string SalesQuotationCRUD(SalesQuotation cy)
        {
            ...
            try
            {
                ...
                using (OracleConnection objConn = new OracleConnection(_connectionString))
                {
                    ...
                    objCmd.Parameters.Add("ENQ_DATE", OracleDbType.Date).Value = DateTime.Parse(cy.EnDate);
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select SALES_QUOTE.BRANCHID,SALES_QUOTE.QUOTE_NO,to_char(SALES_QUOTE.QUOTE_DATE,'dd-MON-yyyy')QUOTE_DATE,SALES_QUOTE.ENQNO,to_char(SALES_QUOTE.ENQ_DATE,'dd-MON-yyyy')ENQ_DATE,SALES_QUOTE.CURRENCY_TYPE,SALES_QUOTE.CUSTOMER,SALES_QUOTE.CUSTOMER_TYPE,SALES_QUOTE.ADDRESS,SALES_QUOTE.CITY,SALES_QUOTE.PINCODE,SALES_QUOTE.CONTACT_PERSON_MAIL,SALES_QUOTE.CONTACT_PERSON_MOBILE,SALES_QUOTE.PRIORITY,SALES_QUOTE.ASSIGNED_TO,ID from SALES_QUOTE  where SALES_QUOTE.SALESQUOTEID=" + id + "";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        public string QuotetoOrder(string id)
        {
            ...
            try
            {
                ...
                using (OracleConnection objConn = new OracleConnection(_connectionString))
                {
                    ...
                    string currency = datatrans.GetDataString("Select CURRENCY_TYPE from SALES_QUOTE where SALES_QUOTE.SALESQUOTEID='" + id + "' ");
