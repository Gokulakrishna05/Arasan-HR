...
namespace Arasan.Services.Store_Management
{
    public class DirectDeductionService : IDirectDeductionService
    {
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select BRANCHID,LOCID,DOCID,to_char(DOCDATE,'dd-MON-yyyy')DOCDATE,DCNO,REASON,GROSS,ENTBY,NARRATION,NOOFD,DEDBASICID  from DEDBASIC where DEDBASICID=" + id + "";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        public string StatusActChange(string tag, string id)
        {
            ...
            try
            {
                ...
                using (OracleConnection objConnT = new OracleConnection(_connectionString))
                {
                    svSQL = "UPDATE DEDBASIC SET IS_ACTIVE ='Y' WHERE DEDBASICID='" + id + "'";
                    OracleCommand objCmds = new OracleCommand(svSQL, objConnT);
                    objConnT.Open();
                    objCmds.ExecuteNonQuery();
                    objConnT.Close();
        ...
        public DataTable GetAllListDirectDeductionItems(string strStatus)
        {
            ...
            {
                SvSql = "Select BRANCHMAST.BRANCHID,LOCDETAILS.LOCID,DOCID,to_char(DEDBASIC.DOCDATE,'dd-MON-yyyy') DOCDATE,DCNO,REASON,GROSS,ENTBY,NARRATION,DEDBASICID,DEDBASIC.IS_ACTIVE  from DEDBASIC LEFT OUTER JOIN BRANCHMAST ON BRANCHMASTID=DEDBASIC.BRANCHID LEFT OUTER JOIN LOCDETAILS ON LOCDETAILS.LOCDETAILSID=DEDBASIC.LOCID Where DEDBASIC.IS_ACTIVE='Y' ORDER BY DEDBASIC.DEDBASICID DESC";
            }
            else
            {
                SvSql = "Select BRANCHMAST.BRANCHID,LOCDETAILS.LOCID,DOCID,to_char(DEDBASIC.DOCDATE,'dd-MON-yyyy') DOCDATE,DCNO,REASON,GROSS,ENTBY,NARRATION,DEDBASICID,DEDBASIC.IS_ACTIVE  from DEDBASIC LEFT OUTER JOIN BRANCHMAST ON BRANCHMASTID=DEDBASIC.BRANCHID LEFT OUTER JOIN LOCDETAILS ON LOCDETAILS.LOCDETAILSID=DEDBASIC.LOCID Where DEDBASIC.IS_ACTIVE='N' ORDER BY DEDBASIC.DEDBASICID DESC";
            }
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
        ...
        public DataTable GetDDByName(string id)
        {
            ...
            SvSql = "Select BRANCHMAST.BRANCHID,LOCDETAILS.LOCID,DOCID,to_char(DOCDATE,'dd-MON-yyyy')DOCDATE,DCNO,REASON,GROSS,MATSUPP,NET,ENTBY,NARRATION,NOOFD,DEDBASICID  from DEDBASIC LEFT OUTER JOIN BRANCHMAST ON BRANCHMASTID=DEDBASIC.BRANCHID LEFT OUTER JOIN LOCDETAILS ON LOCDETAILSID=DEDBASIC.LOCID  \r\n where DEDBASICID=" + id + "";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
        ...
        public DataTable GetDDItem(string id)
        {
            ...
            SvSql = "Select ITEMMASTER.ITEMID,DEDDETAIL.QTY,DEDDETAIL.DEDDETAILID,UNIT,RATE,AMOUNT,BINBASIC.BINID,PROCESSID  from DEDDETAIL LEFT OUTER JOIN ITEMMASTER on ITEMMASTER.ITEMMASTERID=DEDDETAIL.ITEMID LEFT OUTER JOIN BINBASIC on BINBASIC.BINBASICID=DEDDETAIL.BINID   where DEDDETAIL.DEDBASICID='" + id + "'";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
