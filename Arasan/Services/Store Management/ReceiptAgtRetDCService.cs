...
namespace Arasan.Services.Store_Management
{
    public class ReceiptAgtRetDCService : IReceiptAgtRetDC
    {
        ...
        {
            string SvSql = string.Empty;
            SvSql = " SELECT LOCID,DCNO,to_char(RECDCBASIC.DCDATE,'dd-MON-yyyy')DCDATE,REFNO,to_char(RECDCBASIC.REFDATE,'dd-MON-yyyy')REFDATE,STKTYPE,NARRATION,EBY,PARTYID,DOCID,to_char(RECDCBASIC.DOCDATE,'dd-MON-yyyy')DOCDATE FROM RECDCBASIC WHERE RECDCBASICID = '" + id + "' ";
        ...
        {
            string SvSql = string.Empty;
            SvSql = " SELECT ITEMID,BINID,QTY,PENDQTY,REJQTY,UNITMAST.UNITID,SERIALYN,ACCQTY,RATE,AMOUNT FROM RECDCDETAIL LEFT OUTER JOIN UNITMAST on UNITMAST.UNITMASTID = RECDCDETAIL.UNIT WHERE RECDCBASICID = '" + id + "' ";
