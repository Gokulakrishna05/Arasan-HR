...
namespace Arasan.Services
{
    public class StockInService : IStockIn
    {
                                ...
                                for (int i = 0; i < dt.Rows.Count; i++)
                                {
                                    ...
                                    if (rqty >= qty)
                                    {
                                        ...
                                        using (OracleConnection objConnI = new OracleConnection(_connectionString))
                                        {
                                            ...
                                            objCmdI.Parameters.Add("TSOURCEBASICID", OracleDbType.NVarchar2).Value = "0";
                                            ...
                                            objCmdI.Parameters.Add("LOCID", OracleDbType.NVarchar2).Value = cp.LocationID;
                                            ...
                                            using (OracleConnection objConnIn = new OracleConnection(_connectionString))
                                            {
                                                ...
                                                objCmdIn.Parameters.Add("TSOURCEBASICID", OracleDbType.NVarchar2).Value = "0";
        ...
        public DataTable GetAllListStockItem()
        {
            string SvSql = string.Empty;
            SvSql = "Select SUM(INVENTORY_ITEM.BALANCE_QTY) as QTY,INVENTORY_ITEM_ID,ITEMMASTER.ITEMID,UNITMAST.UNITID,BRANCHMAST.BRANCHID,LOCDETAILS.LOCID,INVENTORY_ITEM.ITEM_ID from INVENTORY_ITEM LEFT OUTER JOIN ITEMMASTER on ITEMMASTER.ITEMMASTERID=INVENTORY_ITEM.ITEM_ID LEFT OUTER JOIN UNITMAST ON UNITMAST.UNITMASTID=ITEMMASTER.PRIUNIT LEFT OUTER JOIN BRANCHMAST ON BRANCHMASTID=INVENTORY_ITEM.BRANCH_ID LEFT OUTER JOIN LOCDETAILS ON LOCDETAILS.LOCDETAILSID=INVENTORY_ITEM.LOCATION_ID WHERE INVENTORY_ITEM.BALANCE_QTY !=0 AND INVENTORY_ITEM.LOCATION_ID='10001000000827' GROUP BY ITEMMASTER.ITEMID,UNITMAST.UNITID,BRANCHMAST.BRANCHID,LOCDETAILS.LOCID,INVENTORY_ITEM.ITEM_ID,INVENTORY_ITEM_ID ";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
