                        ...
                        if (dt.Rows.Count > 0)
                        {
                            for (int i = 0; i < dt.Rows.Count; i++)
                            {
                                using (OracleConnection objConnT = new OracleConnection(_connectionString))
                                {
                                    ...
                                    Sql = "Update INVENTORY_ITEM SET  BALANCE_QTY='" + cp.StockQty + "' WHERE INVENTORY_ITEM_ID='"+ dt.Rows[i]["INVENTORY_ITEM_ID"].ToString() + "'";
                                ...
                                using (OracleConnection objConnI = new OracleConnection(_connectionString))
                                {
                                    ...
                                    objCmdI.Parameters.Add("REC_GOOD_QTY", OracleDbType.NVarchar2).Value = cy.QtyS;
                                    objCmdI.Parameters.Add("BALANCE_QTY", OracleDbType.NVarchar2).Value = cy.QtyS;
                                    ...
                                    using (OracleConnection objConnIn = new OracleConnection(_connectionString))
                                    {
                                        ...
                                        objCmdIn.Parameters.Add("TRANS_QTY", OracleDbType.NVarchar2).Value = cy.QtyS;
