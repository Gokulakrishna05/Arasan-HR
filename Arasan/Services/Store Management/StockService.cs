...
namespace Arasan.Services
{
    public class StockService : IStockService
    {
        ...
        {
            string SvSql = string.Empty;
            SvSql = "Select SUM(DECODE(STOCKVALUE.PlusOrMinus,'p',STOCKVALUE.qty,-STOCKVALUE.qty)) stk,ITEMMASTER.ITEMID  ,LOCDETAILS.LOCID from STOCKVALUE LEFT OUTER JOIN ITEMMASTER on ITEMMASTER.ITEMMASTERID=STOCKVALUE.ITEMID  LEFT OUTER JOIN LOCDETAILS ON LOCDETAILS.LOCDETAILSID=STOCKVALUE.LOCID HAVING SUM(DECODE(STOCKVALUE.PlusOrMinus,'p',STOCKVALUE.qty,-STOCKVALUE.qty)) > 0 GROUP BY ITEMMASTER.ITEMID,LOCDETAILS.LOCID";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
