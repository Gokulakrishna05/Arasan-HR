...
namespace Arasan.Services.Store_Management
{
    public class SubContractingDCService : ISubContractingDC
    {
        ...
        {
            string SvSql = string.Empty;
            SvSql = "select ITEMMASTER.ITEMID,ITEM_ID as item FROM INVENTORY_ITEM LEFT OUTER JOIN ITEMMASTER ON ITEMMASTER.ITEMMASTERID=INVENTORY_ITEM.ITEM_ID  WHERE BALANCE_QTY>0 and LOCATION_ID='" + value+"' UNION select ITEMMASTER.ITEMID,DRUM_STOCK.ITEMID as item FROM DRUM_STOCK LEFT OUTER JOIN ITEMMASTER ON ITEMMASTER.ITEMMASTERID=DRUM_STOCK.ITEMID  WHERE BALANCE_QTY>0 and LOCID='"+ value+"'";
            DataTable dtt = new DataTable();
            OracleDataAdapter adapter = new OracleDataAdapter(SvSql, _connectionString);
            OracleCommandBuilder builder = new OracleCommandBuilder(adapter);
            adapter.Fill(dtt);
                            ...
                            if (cp.Isvalid == "Y" && cp.ItemId != "0")
                            {
                                 ...
                                 if (cp.Drumsdesc!="" && cp.Drumsdesc!=null)
                                {
                                    ...
                                    if (Ddrum.Length > 0)
                                    {
                                        ...
                                        if (dddrum == null || dddrum == "")
                                        {
                                            DataTable dt = datatrans.GetData("Select INVENTORY_ITEM.BALANCE_QTY,INVENTORY_ITEM.ITEM_ID,INVENTORY_ITEM.LOCATION_ID,INVENTORY_ITEM.BRANCH_ID,INVENTORY_ITEM_ID,GRNID,GRN_DATE,LOT_NO from INVENTORY_ITEM where ITEM_ID='" + cp.ItemId + "' AND LOT_NO='" + ddlot + "' and LOCATION_ID='" + ss.Location + "'");
                                            ...
                                            if (dt.Rows.Count > 0)
                                            {
                                                ...
                                                double dqty = Convert.ToDouble(ddqty); 
                                        ...
                                        {
                                            string drum = datatrans.GetDataString("Select DRUMNO from DRUMMAST where DRUMMASTID='" + dddrum + "'");

                                            DataTable dt = datatrans.GetData("Select DRUM_STOCK.BALANCE_QTY,DRUM_STOCK.ITEMID,DRUM_STOCK.LOCID,DRUM_STOCK.DRUM_NO,DRUM_ID,DRUM_STOCK_ID,WCID from DRUM_STOCK where DRUM_STOCK.ITEMID='" + cp.ItemId + "' AND DRUM_STOCK.LOCID='" + ss.Location + "' and DRUM_NO='" + dddrum + "' and BALANCE_QTY!=0 order by DOC_DATE ASC");
                                            if (dt.Rows.Count > 0)
                                            {
                                                double dqty = Convert.ToDouble(ddqty);
                                                ...
                                                objCmdIn.Parameters.Add("DRUMNO", OracleDbType.NVarchar2).Value = dddrum;
                                                objCmdIn.Parameters.Add("DRUM", OracleDbType.NVarchar2).Value = drum;
                                                ...
                                                objCmdIn.Parameters.Add("RATE", OracleDbType.NVarchar2).Value = "0";
                                                objCmdIn.Parameters.Add("LOTNO", OracleDbType.NVarchar2).Value = ddlot;
        ...
        public string PackMatSubConDCCRUD(SubContractingDC cy)
        {
            ...
            try
            {
                ...
                using (OracleConnection objConn = new OracleConnection(_connectionString))
                {
                    ...
                    objCmd.Parameters.Add("DCREFID", OracleDbType.NVarchar2).Value = cy.ID;
        ...
        public async Task<IEnumerable<Subcondcdet>> GetSubcondc(string id )
        {
            ...
            {
                return await db.QueryAsync<Subcondcdet>(" SELECT  TAAIERP.SUBCONTDCBASIC.SUBCONTDCBASICID, TAAIERP.SUBCONTDCBASIC.BRANCH, to_char(SUBCONTDCBASIC.DOCDATE,'dd-MM-yy')DOCDATE, TAAIERP.SUBCONTDCBASIC.DOCID, TAAIERP.SUBCONTDCBASIC.REFNO, TAAIERP.SUBCONTDCBASIC.WCID, TAAIERP.SUBCONTDCBASIC.FROMLOCATION, TAAIERP.SUBCONTDCBASIC.ENTEREDBY, TAAIERP.SUBCONTDCBASIC.NARRATION, TAAIERP.SUBCONTDCBASIC.LOCID, TAAIERP.SUBCONTDCBASIC.TOTQTY, TAAIERP.SUBCONTDCBASIC.TOTRECQTY, TAAIERP.SUBCONTDCBASIC.EBY, TAAIERP.SUBCONTDCBASIC.USERID, PARTYMAST.PARTYID,  TAAIERP.SUBCONTDCBASIC.PARTYMASTID, TAAIERP.SUBCONTDCBASIC.TOLOCATION, TAAIERP.SUBCONTDCBASIC.LOCDETAILSID, SUBCONTDCBASIC.ADD1 ||','||SUBCONTDCBASIC.ADD2 ||','||SUBCONTDCBASIC.CITY as ADDRESS, TAAIERP.SUBCONTDCBASIC.THROUGH, TAAIERP.SUBCONTDCDETAIL.SUBCONTDCDETAILID, TAAIERP.SUBCONTDCDETAIL.SUBCONTDCBASICID AS EXPR2, ITEMMASTER.ITEMID,ITEMMASTER.HSN, TAAIERP.SUBCONTDCDETAIL.UNIT, TAAIERP.SUBCONTDCDETAIL.QTY,SUBCONTDCDETAIL.NOOFBAGS,PARTYMAST.GSTNO, TAAIERP.SUBCONTDCDETAIL.RATE, TAAIERP.SUBCONTDCDETAIL.AMOUNT, TAAIERP.SUBCONTEDET.SUBCONTEDETID,TAAIERP.SUBCONTEDET.SUBCONTDCBASICID AS EXPR3, TAAIERP.SUBCONTEDET.RITEM, TAAIERP.SUBCONTEDET.RITEMMASTERID, TAAIERP.SUBCONTEDET.RUNIT, TAAIERP.SUBCONTEDET.ERATE, TAAIERP.SUBCONTEDET.EAMOUNT, TAAIERP.SUBCONTEDET.ERQTY, TAAIERP.SUBCONTEDET.APPRATE, TAAIERP.SUBCONTEDET.EFRATE, TAAIERP.SUBCONTEDET.RSUBQTY, TAAIERP.SUBCONTEDET.RECQTY FROM TAAIERP.SUBCONTDCBASIC LEFT OUTER JOIN PARTYMAST ON PARTYMAST.PARTYMASTID=SUBCONTDCBASIC.PARTYID INNER JOIN PARTYMAST ON PARTYMAST.PARTYMASTID =SUBCONTDCBASIC.PARTYID INNER JOIN TAAIERP.SUBCONTDCDETAIL ON TAAIERP.SUBCONTDCBASIC.SUBCONTDCBASICID = TAAIERP.SUBCONTDCDETAIL.SUBCONTDCBASICID LEFT OUTER JOIN ITEMMASTER ON ITEMMASTER.ITEMMASTERID=SUBCONTDCDETAIL.ITEMID INNER JOIN TAAIERP.SUBCONTEDET ON TAAIERP.SUBCONTDCBASIC.SUBCONTDCBASICID = TAAIERP.SUBCONTEDET.SUBCONTDCBASICID INNER JOIN ITEMMASTER ON ITEMMASTER.ITEMMASTERID =SUBCONTDCDETAIL.ITEMID  where SUBCONTDCBASIC.SUBCONTDCBASICID='" + id + "' and SUBCONTEDET.SUBCONTDCBASICID ='" + id + "' and SUBCONTDCDETAIL.SUBCONTDCBASICID ='" + id + "'", commandType: CommandType.Text);
            }
        }
        ...
        public async Task<IEnumerable<Subcondcdetet>> GetSubcondcdet(string id)
        {
            using (OracleConnection db = new OracleConnection(_connectionString))
            {
                return await db.QueryAsync<Subcondcdetet>(" SELECT  SUBCONTEDET.SUBCONTEDETID,TAAIERP.SUBCONTEDET.SUBCONTDCBASICID AS EXPR3, ITEMMASTER.ITEMID, TAAIERP.SUBCONTEDET.RITEMMASTERID, TAAIERP.SUBCONTEDET.RUNIT, TAAIERP.SUBCONTEDET.ERATE, TAAIERP.SUBCONTEDET.EAMOUNT, TAAIERP.SUBCONTEDET.APPRATE, TAAIERP.SUBCONTEDET.EFRATE, TAAIERP.SUBCONTEDET.RSUBQTY, TAAIERP.SUBCONTEDET.ERQTY,ITEMMASTER.HSN FROM TAAIERP.SUBCONTDCBASIC INNER JOIN TAAIERP.SUBCONTEDET ON TAAIERP.SUBCONTDCBASIC.SUBCONTDCBASICID = TAAIERP.SUBCONTEDET.SUBCONTDCBASICID LEFT OUTER JOIN ITEMMASTER ON ITEMMASTER.ITEMMASTERID=SUBCONTEDET.RITEM INNER JOIN ITEMMASTER ON ITEMMASTER.ITEMMASTERID = TAAIERP.SUBCONTEDET.RITEM where SUBCONTEDET.SUBCONTDCBASICID ='" + id + "'", commandType: CommandType.Text);
            }
        }
