...
namespace Arasan.Services.Transaction
{
    public class MissingPunchEntryService : IMissingPunchEntry
    {
        ...
        public DataTable GetAlLMissingPunchEntry(string strStatus)
        {
            ...
            {
                SvSql = "SELECT ID,EMPMAST.EMPNAME,to_char(ATTENDANCE_DATE,'dd-MON-yyyy')ATTENDANCE_DATE,REASON,DEVICE_ID,MISSING_IN_OUT,MISSING_IN,MISSING_OUT,STATUS FROM HRM_MISSING_PUNCH LEFT OUTER JOIN EMPMAST ON EMPMAST.EMPMASTID=HRM_MISSING_PUNCH.EMPLOYEE_ID";
